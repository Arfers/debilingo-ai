...
app.post("/check", async (req, res) => {
  ...
  try {
    const completion = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: "Ты — интеллектуальный помощник языкового сайта для начинающих. Оцени развернутый ответ ученика по 5-балльной шкале, строго по критериям: ясность, грамматика, словарный запас. Форматируй ответ следующим образом: 1. Общая оценка: X/5; 2. Ошибки: (маркированным списком кратко и понятно описывай каждую ошибку, одна ошибка — один пункт. всегда указывай, как правильно); 3. Комментарий: (краткое общее замечание — только если необходимо); Пиши строго, понятно, без воды. Не используй сложные термины. Пиши с разметкой HTML, чтобы сайт мог красиво отобразить ответ, и чтобы пользователь видел красивый маркированный пронумерованный список." },
